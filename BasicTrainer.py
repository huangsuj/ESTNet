...
class Trainer(object):
    ...
    def multi_train(self):
        ...
        if self.args.val_ratio > 0:
            # print('epoch:', epoch)
            # print('Lr:{:.2E}', self.optimizer.state_dict()['param_groups'][0]['lr'])
            self.model.load_state_dict(best_model)
            mae = self.test(self.model, self.args, self.scaler_dict, self.test_dataloader, self.logger)

            if not os.path.isdir('./weights/' + self.args.dataset_use[0]):
                os.mkdir('./weights/' + self.args.dataset_use[0])
            model_path = './weights/' + self.args.dataset_use[0]
            self.save_model(model_path, epoch, best_model, self.optimizer, self.scheduler)

        return mae



    def save_model(self, save_path, epoch, best_model_state_dict, optimizer, lr_schedule):
        checkpoint = {'epoch': epoch,
                    'optimizer_state_dict': optimizer.state_dict(),
                    'model_state_dict': best_model_state_dict,
                    # 'lr_schedule': lr_schedule.state_dict()
                      }
        torch.save(checkpoint, save_path + '/ckpt_best_%s.pth' %(str(epoch)))
    ...
    def multi_train_eps(self):
        ...
        for inputs, targets in self.train_dataloader:
            ...
            out = self.model(inputs, targets, select_dataset)
