...
def load_st_dataset(dataset, args):
    ...
    #   9/1/2018 - 11/30/2018 Saturday
        ...
        data_path = os.path.join(f'/gpu-data/hsj/opencity_main/data/{dataset}/{dataset}.npz')
        data = np.load(data_path)['data'][:, :, 0]
        week_start = 6
        interval = 5
        week_day = 7
    ...
    # 3 / 5 / 2022 - 4 / 5 / 2022
    elif dataset == 'TrafficJN':
        data_path = os.path.join(f'/gpu-data/hsj/dynamic_datasets/{dataset}/{dataset}.npz')
    ...
    # 5 / 1 / 2012 - 6 / 30 / 2012 Tuesday
        ...
        data_path = os.path.join('/gpu-data/hsj/dynamic_datasets/PEMS07M/PEMS07M.npz')
        data = np.load(data_path)['data']
        week_start = 2
        weekday_only = True
        interval = 5
    ...
    elif dataset == 'CAD3' or dataset == 'CAD5':
        data_path = os.path.join(f'/gpu-data/hsj/OpenCity_ori/data/{dataset}/{dataset}.npz')
        data = np.load(data_path)['data'][:288 * (31+29+15), :]
        data[np.isnan(data)] = 0
        week_start = 3
        interval = 5
    ...
    # 1 / 1 / 2019 - 12 / 31 / 2019
        ...
        data_path = os.path.join(f'/gpu-data/hsj/dynamic_datasets/{dataset}/{dataset}.h5')
        import pandas as pd
...
class TrafficDataset(Dataset):
    def __init__(self, data, batch_size, input_window=288, output_window=288, eval_only=False):
        ...
        # preprocess
        ### 记录所有输入窗口(input_win, nodes, features)和输出窗口(output_win, nodes, features)的数据
        self.windows = [
        ...
        # batch
        # 原来的训练数据一共要滑动窗口5760次(len(self.windows))，分batch是为了不要一次性将5760全部用于训练
        # 而是每次64次滑动数据用于训练
        self.batches = [
...
def define_dataloder(args):
    ...
    for dataset_name in args.dataset_use:
        ...
        if args.real_value == False:
            # 获得所有训练数据在三个特征维度的均值和方差[domain, time of day, day of week]
        ...
        if dataset_name.startswith("Traffic") or dataset_name.startswith("NYC") or dataset_name.startswith("CHI"):
            ...
            intervel = 10
        elif 'GBA' in dataset_name or 'CAD3' in dataset_name or 'CAD5' in dataset_name:
            intervel = 15
        else:
            intervel = 5
        iw = args.his // (intervel // 5) # //返回商，向下取整
